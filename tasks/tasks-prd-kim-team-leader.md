...
- [x] **1.0 프로젝트 초기 설정 및 인프라 구축**
    - [x] 1.1 Next.js 14 프로젝트 생성 (App Router, TypeScript)
    - [x] 1.2 Tailwind CSS 및 shadcn/ui 설치 및 설정
    - [x] 1.3 Zustand 상태 관리 라이브러리 설치 및 기본 스토어 구조 설정
    - [x] 1.4 Prisma ORM 설치 및 PostgreSQL 연결 설정
    - [x] 1.5 데이터베이스 스키마 설계 및 마이그레이션 (User, Team, Agent, Persona, Task, Result, History)
    - [x] 1.6 NextAuth.js 설치 및 이메일/소셜 로그인 설정 (Google, GitHub)
    - [x] 1.7 인증 미들웨어 구현 (보호된 라우트 설정)
    - [x] 1.8 로그인/회원가입 페이지 UI 구현
    - [x] 1.9 공통 레이아웃 컴포넌트 구현 (Header, Sidebar, Layout)
    - [x] 1.10 공통 UI 컴포넌트 설정 (Button, Card, Modal, Input 등 shadcn/ui 컴포넌트)
    - [x] 1.11 환경 변수 설정 (.env.local - DB, API 키 등)
    - [x] 1.12 API 클라이언트 유틸리티 설정 (fetch wrapper, 에러 핸들링)

- [x] **2.0 팀/부서 관리 기능 개발**
    - [x] 2.1 팀 데이터 모델 정의 및 Prisma 스키마 작성
    - [x] 2.2 팀 CRUD API 엔드포인트 구현 (GET, POST, PUT, DELETE)
    - [x] 2.3 팀 목록 페이지 UI 구현 (카드 그리드 뷰)
    - [x] 2.4 팀 카드 컴포넌트 구현 (이름, 설명, 팀원 수 표시)
    - [x] 2.5 팀 생성 모달 구현 (이름, 설명, 목적 입력 폼)
    - [x] 2.6 팀 수정 모달 구현 (기존 정보 편집)
    - [x] 2.7 팀 삭제 기능 구현 (확인 다이얼로그 포함)
    - [x] 2.8 팀 상세/대시보드 페이지 구현 (팀 개요, 활성 작업, 팀원 현황)
    - [x] 2.9 팀 전환 드롭다운 컴포넌트 구현 (헤더에 배치)
    - [x] 2.10 범용 팀 템플릿 데이터 생성 및 적용 로직 구현
    - [x] 2.11 팀 상태 관리 스토어 구현 (Zustand - 현재 팀, 팀 목록)
